...
. "/Users/laszlo/.no-mistakes/worktrees/b44e88b6dda0/01M3CQVZAA1QWDNJWFXWB7Z6KJ/tests/lib.sh"
...
show() { echo "---- $1: rc=$2"; sed 's/^/  stderr| /' "$3/stderr"; echo "  glab merge line: $(glab_merge_line "$3/glab.log")"; }

# Scenario: attended auto-merge / merge-train path on a merged-results pipeline.
test_gitlab_merged_results_auto_merge_attended() {
  local case_dir rc merge_line
  case_dir=$(make_gitlab_case gitlab-mr-auto \
    "pipeline_sha=$MR_MERGE_SHA" "pipeline_ref=refs/merge-requests/7/merge")
  write_merged_results_fixtures "$case_dir" "$MR_TARGET_HEAD" "$MR_HEAD"
  set +e
  ...
  rc=$?
  set -e
  show auto-merge-attended "$rc" "$case_dir"
  merge_line=$(glab_merge_line "$case_dir/glab.log")
  case "$merge_line" in *"--sha $MR_HEAD"*"--auto-merge"*) : ;; *) fail "auto: merge line '$merge_line'" ;; esac
  assert_grep "merged-results pipeline at $MR_MERGE_SHA proving head $MR_HEAD" "$case_dir/stderr" "auto: acceptance not reported"
  pass "attended --auto-merge accepts a merged-results pipeline"
}

# Transcript cases for the other scenarios (print what an operator sees).
transcript() {
  local name=$1 ref=$2 p0=$3 p1=$4 extra=${5:-} case_dir rc
  case_dir=$(make_gitlab_case "tx-$name" "pipeline_sha=$MR_MERGE_SHA" "pipeline_ref=$ref")
  write_merged_results_fixtures "$case_dir" "$p0" "$p1"
  [ -z "$extra" ] || : > "$case_dir/$extra"
  set +e; run_pr_merge "$case_dir" task-x1 "$MR_URL" > "$case_dir/stdout" 2> "$case_dir/stderr"; rc=$?; set -e
  show "$name" "$rc" "$case_dir"
}


BASE_ROOT=/Users/laszlo/.no-mistakes/evidence/01M3CQVZAA1QWDNJWFXWB7Z6KJ/base
REAL_GLAB=/etc/profiles/per-user/laszlo/bin/glab
LIVE_BASE=https://gitlab.com/teamworksdev/performance/data-gen-ai/ingestion/ingestion
live_case() {
  local label=$1 root=$2 iid=$3 case_dir rc
  case_dir=$(make_gitlab_case "live-$label-$iid")
  # Proxy to the real, logged-in glab; only the mutating "mr merge" is blocked.
  cat > "$case_dir/fakebin/glab" <<SH
#!/usr/bin/env bash
printf '%s\n' "\$*" >> "$case_dir/glab-live.log"
if [ "\$1" = mr ] && [ "\$2" = merge ]; then echo "BLOCKED BY TEST HARNESS: would run glab \$*" >&2; exit 1; fi
HOME=/Users/laszlo exec "$REAL_GLAB" "\$@"
SH
  chmod +x "$case_dir/fakebin/glab"
  set +e
  ROOT=$root PR_MERGE=$root/bin/fm-pr-merge.sh run_pr_merge "$case_dir" task-x1 "$LIVE_BASE/-/merge_requests/$iid" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
  echo "==== [$label script] real GitLab MR !$iid: rc=$rc"
  sed 's/^/  stderr| /' "$case_dir/stderr"
  echo "  glab calls:"; sed 's/^/    glab /' "$case_dir/glab-live.log"
}
for iid in 734 739; do
  live_case base "$BASE_ROOT" "$iid"
  live_case fixed "$ROOT" "$iid"
done
